...
class HopfieldSolver():
    def __init__(self, activation_type='sin',
        ...
        self.inverse_activation_function = getattr(utils,
                                                   'inverse_activation_' +
                                                   activation_type)
        self.proxy_distance_vector = getattr(utils,
                                             'proxy_distance_vector_' +
                                             activation_type)
    ...
    def setup_optimization_problem(self, objective_function, gradient, lb, ub,
        ...
        if type(beta) == int:
            ...
            self.beta = beta
        problem['x_0'] = self._compute_x_0(x_0)
        return problem
    ...
    def solve_optimization_problem(self, problem):
        ...
        if self.problem['A_ineq'] and self.problem['b_ineq'] and self.problem[
                'A_eq'] and self.problem['b_eq']:

            def inequality_constraint(optimization_variable, slack_variable):
                return np.dot(self.problem['A_ineq'],
                              optimization_variable)
                - self.problem['b_ineq'] - slack_variable
            ...
            def equality_constraint(optimization_variable):
                ...
                def objective_function(variables):
                    ...
                    main_function = self.problem['objective_function'](
                        optimization_variable)
                    inequality_term = np.dot(
                        dual_variable_ineq.T,
                    ...
                    equality_term = np.dot(dual_variable_eq.T, equality_constraint(optimization_variable)) + self.problem[
                        'penalty_eq'] / 2 * np.linalg.norm(equality_constraint(
            ...
            def gradient(variables):
                ...
                inequality_term = np.dot(self.problem['A_ineq'].T, dual_variable_ineq) + self.problem[
                    'penalty_ineq'] * np.dot(
                        self.problem['A_ineq'].T,
                        inequality_constraint(optimization_variable, slack_variable))
                return main_function + equality_term + inequality_term

            def gradient_wrt_slack_variable(variables):
                optimization_variable, slack_variable = variables
                return - self.problem['penalty_ineq'] * inequality_constraint(optimization_variable, slack_variable) - dual_variable_ineq

        elif self.problem['A_ineq'] and self.problem['b_ineq'] and (
                self.problem['A_eq'] or self.problem['b_eq']):
            ...
            def gradient(variables):
                ...
                main_function = self.problem['gradient'](optimization_variable)
                inequality_term = np.dot(self.problem['A_ineq'].T, dual_variable_ineq) + self.problem[
                    'penalty_ineq'] * np.dot(self.problem['A_ineq'].T, inequality_constraint(optimization_variable, slack_variable))
                return main_function + inequality_term
        ...
        x_h[:, 0] = self._inverse_activation(
            self.problem['x_0'], self.problem['lb'], self.problem['ub'])
        if self.problem['A_ineq'] is not None and self.problem['b_ineq'] is not None:
            s = np.nan * \
                np.ones((self.problem['dim_problem'], self.max_iterations))
        ...
        if np.linalg.norm(grad_f) == 0:
            grad_f = (self.problem['smoothness_coef'] / 10) * \
                (np.random.rand(self.problem['dim_problem']) - 0.5)
        k = 0
    ...
    def _solve_dual_gradient_ascent(self):
        ...
        if self.problem['A_ineq'] is not None and self.problem['b_ineq'] is not None and self.problem[
            ...
            lb = np.concatenate(
                (self.problem['lb'].copy(), - np.inf * np.ones((n,))), axis=0)
            ub = np.concatenate(
                (self.problem['ub'].copy(), np.zeros((n,))), axis=0)
            bounds = Bounds(lb, ub)
            ...
            def dual_function(variables):
                return self.problem['objective_function'](variables[:n]) + self.problem[
            ...
            result = minimize(dual_function, x0, method='trust-constr',
                              constraints=(
                                  {"fun": inequality_constraint, "type": "eq"}),
                              options={'verbose': 1}, bounds=bounds)
            dual_variable_ineq = result.v[0]
            ...
            result = minimize(dual_function, x0, method='trust-constr',
                              constraints=(
                                  {"fun": equality_constraint, "type": "eq"}),
    ...
    def _alpha_hop(self, x, grad_f, k, direction):
        ...
        if self.direction_type is 'stochastic':
            alpha = (1 - 1 / np.sqrt(k)) * alpha + 1 / \
                (self.problem['smoothness_coef'] * np.sqrt(k))
    ...
    def _compute_x_0(self, x_0):
        ...
        if np.linalg.norm(grad_f) == 0:
            grad_f = (self.problem['smoothness_coef'] /
                      10) * (np.random.rand(n) - 0.5)

        while iterations < max_iterations and utils.is_in_box(x_0, self.problem['ub'] - self.ascent_stop_criterion, self.problem['lb'] + self.ascent_stop_criterion) \
            ...
            if self.initial_ascent_type is 'ascent':
                ...
                x_0 = x_0 + (1 / self.problem['smoothness_coef']) * np.multiply(
                    grad_f, np.ones(n) - self.problem['binary_indicator'])
            iterations += 1
    ...
    def _find_direction(self, x, grad_f):
        ...
        if self.direction_type is 'classic' or self.direction_type is 'stochastic':
            ...
            y = max(0, - np.dot(g.T, w) + math.atan(self.theta) *
                    np.sqrt(np.linalg.norm(w) ** 2 - np.dot(g.T, w) ** 2))
